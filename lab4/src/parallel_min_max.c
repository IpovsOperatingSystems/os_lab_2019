...
int main(int argc, char **argv) {
  ...
  // Инициализация активных дочерних процессов
  int active_child_processes = 0;

  struct timeval start_time;
  gettimeofday(&start_time, NULL);

  // Создаем каналы синхронизации процессов
  ...
  for (int i = 0; i < pnum; i++) {
    ...
    if (child_pid >= 0) {
      ...
      if (child_pid == 0) {
        ...
        int mini_array_size = array_size / pnum;
        int begin = i * mini_array_size;
        int end;

        if (i == pnum - 1) {
            end = array_size - 1;
        } else {
            end = (i + 1) * mini_array_size;
        }
        // parallel somehow

         struct MinMax array_minmax = GetMinMax(array, begin, end);
        ...
        if (with_files) {
          ...
          fprintf(file, "%d %d\n", array_minmax.min, array_minmax.max);

          fclose(file);
        } else {
          // use pipe here
              close(pipes[i][0]); // Закрыть чтение из pipe
              write(pipes[i][1], &array_minmax, sizeof(struct MinMax));
  ...
  for (int i = 0; i < pnum; i++) {
    ...
    if (with_files) {
      ...
      // read from pipes
      close(pipes[i][1]); // Закрыть запись в pipe
      struct MinMax array_minmax;
      read(pipes[i][0], &array_minmax, sizeof(struct MinMax));
      close(pipes[i][0]); // Закрыть чтение из pipe

      if (array_minmax.min < min_max.min) min_max.min = array_minmax.min;
      if (array_minmax.max > min_max.max) min_max.max = array_minmax.max;
    }
